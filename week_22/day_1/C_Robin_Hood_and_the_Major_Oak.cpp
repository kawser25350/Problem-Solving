 ...
 * File:   C_Robin_Hood_and_the_Major_Oak.cpp
 */
#include <bits/stdc++.h>
...
// odd^ood ==odd  even^even==even so dont need to do pow(i,i)
   ll leaves=n-k; // koita year ar leaves bad jabe 
   ll leavessum=leaves*(leaves+1)/2;
  

   
   ll sum=n*(n+1)/2;

   ll totalsum= sum-leavessum;

   //cout<<leaves<<" "<<leavessum<<" "<<sum<<ln;



   cout<<(totalsum % 2  ==0 ? "yes":"no")<<ln;
